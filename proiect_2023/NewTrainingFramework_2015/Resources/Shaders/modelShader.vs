...
void main()
{
	...
	vec4 normL = vec4(a_norm, 0.0);
	vec4 world_norm = u_model * normL;

	vec4 binormL = vec4(a_binorm, 0.0);
	vec4 world_binorm = u_model * binormL;

	vec4 tgtL = vec4(a_tgt, 0.0);
	vec4 world_tgt = u_model * tgtL;
	...
	v_norm = normalize(world_norm.xyz);
	v_world_binorm = normalize(world_binorm.xyz);
	v_world_tgt = normalize(world_tgt.xyz);
}
