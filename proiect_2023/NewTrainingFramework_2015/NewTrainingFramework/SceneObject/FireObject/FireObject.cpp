...
void FireObject::Draw(Camera* cam)
{
	...
	SendCommonData();
	Shader* shader = ResourceManager::GetInstance()->LoadShader(shaderId);
	if (shader->GetShader()->timeUniform != -1)
	{
		glUniform1f(shader->GetShader()->timeUniform, this->time);
	}

	if (shader->GetShader()->dispMaxUniform != -1)
	{
		glUniform1f(shader->GetShader()->dispMaxUniform, this->dispMax);
	}
